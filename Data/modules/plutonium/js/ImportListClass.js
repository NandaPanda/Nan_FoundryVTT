...
class ImportListClass extends ImportListCharacter {
	static ID = "classes-subclasses";
	static DISPLAY_NAME_TYPE_PLURAL = "Classes & Subclasses";
		...
		this._$btnRun.click(async () => {
			...
			const selIds = this._actor
				? this._list.items
					...
					.map(it => ({ixClass: it.data.ixClass, ixSubclass: it.data.ixSubclass}))
				: this._list.items
	...
	async _pImportEntry_pDoUpdateCharacter_spellcasting ({actUpdate, cls, sc, dataBuilderOpts, otherExistingClassItems, otherExistingSubclassItems}) {
		const {
			...
			maxPactCasterLevel,
		} = Charactermancer_Class_Util.getCasterProgression(cls, sc, {targetLevel: dataBuilderOpts.targetLevel, otherExistingClassItems, otherExistingSubclassItems});
	...
	static _getLevelledEmbeddedDocuments ({importSummary, level}) {
		return importSummary.imported
			...
			.map(importMeta => new UtilAdvancements.LevelledEmbeddedDocument({
...
ImportListClass.SheetLevelUpButtonAdapter = class {
	...
	static async _pGetAvailableClassSubclass (itemTuple) {
		const {classItem: existingClassItem, existingSubclassItem} = itemTuple;
