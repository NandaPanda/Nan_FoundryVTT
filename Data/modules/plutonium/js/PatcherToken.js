...
class Patcher_Token {
	...
	static async _pDoTokenFakeDraw ({token}) {
		token._drawHUD();
		token.refresh();
		// These are part of the HUD, but (as of 2022-02-08) unfortunately not drawn by `_drawHUD`
	...
	static _doRemove (token) {
		if (!token._plutonium_xDispDamageDealt) return;
	...
	static _handleConfigUpdate_displayDamageDealt_doAddDisplay (token) {
		...
		token._plutonium_xDispDamageDealt = new PreciseText("", CONFIG.canvasTextStyle.clone());
