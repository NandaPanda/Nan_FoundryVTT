...
class DataConverterOptionalfeature extends DataConverterFeature {
	...
	static async _pGetOptionalFeatureItem_fromSrd (optFeature, opts) {
		...
		const effects = [
			...
			...opts.isActorItem ? [] : (await this._pGetEffectsSideLoaded({ent: feature, img: srdData.img}) || []),
		];
	...
	static async pGetSideLoadedMatch (optFeature) {
		return this._pGetSideLoadedMatch(optFeature, this._SIDE_LOAD_OPTS);
	}
