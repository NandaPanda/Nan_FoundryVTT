...
class DataConverter {
	...
	static async _pGetImagePath (ent, {fluff, propCompendium} = {}) {
		return this._pGetImagePath_(ent, {fluff, propCompendium});
	}

	static async _pGetSaveImagePath (ent, {fluff, propCompendium} = {}) {
		return Vetools.pOptionallySaveImageToServerAndGetUrl(await this._pGetImagePath(ent, {fluff, propCompendium}));
	}

	static async _pGetImagePath_ (ent, {fluff, propCompendium} = {}) {
	...
	static async pGetItemActorPassive (entry, opts) {
		...
		const state = new this._PassiveEntryParseState(entry, opts);
		await state.pInit();

		let name = UtilApplications.getCleanEntityName(UtilDataConverter.getNameWithSourcePart(entry, {displayName: opts.displayName, isActorItem: true}));
		...
		let {
			...
			name: uses_name,
			activationType: uses_activationType,
			activationCondition: uses_activationCondition,
		} = this._pGetItemActorPassive_getUses({
			entry,
			opts,
			name,
			...
			strEntries,
			consumeType: state.consumeType,
		});

		name = uses_name;
		state.activationType = uses_activationType;
		state.activationCondition = uses_activationCondition;
		// endregion
		...
		return {
			...
			data: {
				source: UtilDataConverter.getSourceWithPagePart(fauxEntrySourcePage),
	...
	static _pGetItemActorPassive_getUses ({entry, opts, name, activationType, activationCondition, strEntries, consumeType}) {
		const out = {
			name,
			...
			usesCount: opts.usesCount || 0, // This is always a number
			usesCountMax: opts.usesCountMax || "0", // This can be a formula
			usesPer: opts.usesPer || null, // Foundry currently (2020-04-26) supports "Day", "Short Rest", "Long Rest", and "Charges"
		};

		this._pGetItemActorPassive_getUses_creature({entry, opts, out});
		this._pGetItemActorPassive_getUses_player({entry, opts, out, strEntries, consumeType});

		return out;
	}

	static _pGetItemActorPassive_getUses_creature ({entry, opts, out}) {
		...
		out.name = out.name
			.replace(/\(Recharges after a (?<restPart>[^)]+)\)/i, (...m) => {
				out.usesCount = 1;
				out.usesCountMax = `${out.usesCount}`;
		...
		if (!out.usesPer) {
			out.name = out.name.replace(/\(\s*(\d+)\s*\/\s*(Day|Short Rest|Long Rest)\s*\)/i, (...m) => {
				out.usesCount = Number(m[1]);
				out.usesCountMax = `${out.usesCount}`;

				const cleanTime = m[2].trim().toLowerCase();
				switch (cleanTime) {
					case "day": out.usesPer = "day"; break;
					case "short rest": out.usesPer = "sr"; break;
					case "long rest": out.usesPer = "lr"; break;
				}
		...
		if (!out.usesPer) {
			out.name = out.name.replace(/\(\s*(\d+)\s+Charges\s*\)/i, (...m) => {
				out.usesCount = Number(m[1]);
				out.usesCountMax = `${out.usesCount}`;
				out.usesPer = "charges";
	...
	static _pGetItemActorPassive_getUses_player ({entry, opts, out, strEntries, consumeType}) {
		...
		if (isShortRest) out.usesPer = "sr";
		else if (isLongRest) out.usesPer = "lr";
		// endregion
		...
		strEntries.replace(/you can use ([^.!?]+) a number of times equal to(?<mult> twice)? your proficiency bonus/i, (...m) => {
			const mult = m.last().mult
				...
				: 1;
			out.usesCount = opts.actor ? (opts.actor.getRollData().prof * mult) : null;
			out.usesCountMax = `@prof${mult > 1 ? ` * ${mult}` : ""}`;
		});
		...
		strEntries.replace(/you can use this (?:feature|ability) (?<mult>once|twice|[a-zA-Z]+ times)/i, (...m) => {
			...
			out.usesCount = mult;
			out.usesCountMax = mult;
		});

		// If we found no use count, assume it's one-use
		if (out.usesPer && !out.usesCount && (!out.usesCountMax || out.usesCountMax === "0")) {
			out.usesCount = 1;
			out.usesCountMax = `${out.usesCount}`;
		}
		// endregion
	...
	static async _pGetSideLoadedMatch (ent, {propBrew, fnLoadJson, propJson, propsMatch = ["source", "name"]}) {
		...
		if (propBrew) {
			const found = (MiscUtil.get(BrewUtil, "homebrew", propBrew) || [])
				.find(it => this._pGetAdditional_fnMatch(ent, it, propsMatch));
			if (found) founds.push(found);
		}
	...
	static mutEffectsDisabledTransfer (effects, configGroup) {
		...
		effects.forEach(effect => {
			const isPassiveEffects = this._isPassiveEffect(effect);

			const disabled = Config.get(configGroup, "setEffectDisabled");
			switch (disabled) {
				case ConfigConsts.C_USE_PLUT_VALUE: effect.disabled = isPassiveEffects; break;
			...
			switch (transfer) {
				case ConfigConsts.C_USE_PLUT_VALUE: effect.transfer = isPassiveEffects; break;
	...
	static _isPassiveEffect (effect) {
		if (
