...
`);
	...
	static _getWikiSummary () {
		...
		const getOptions = (setting) => {
			switch (setting.type) {
				...
				case "multipleChoice": {
					return (setting.choices || []).map(choice => ({name: choice}));
				}
		...
		return this._DEFAULT_CONFIG__SORTED.map(([, group]) => {
			return {
	...
	static getCompendiumPaths () {
		const out = [];
		Object.entries(ConfigConsts.DEFAULT_CONFIG_)
			.forEach(([groupKey, group]) => {
				Config._KEYS_SETTINGS_METAS.forEach(prop => {
