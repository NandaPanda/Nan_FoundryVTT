...
class DataConverterVehicle extends DataConverter {
	...
	static _getVehicleSpeed_ship (veh) {
		...
		const perHour = Config.getMetricNumberDistance({configGroup: "importVehicle", originalValue: veh.pace, originalUnit: UNT_MILES});
		const perDay = Config.getMetricNumberDistance({configGroup: "importVehicle", originalValue: veh.pace * 24, originalUnit: UNT_MILES});

		const unit = Config.getMetricUnitDistance({configGroup: "importVehicle", originalUnit: UNT_MILES, isShortForm: false, isPlural: veh.pace !== 1});

		return `${perHour} ${unit} per hour (${perDay} ${unit} per day)`;
	}

	static _getVehicleSpeed_infwar (veh) {
		const speed = Config.getMetricNumberDistance({configGroup: "importVehicle", originalValue: veh.speed, originalUnit: "ft."});
		const unit = Config.getMetricUnitDistance({configGroup: "importVehicle", originalUnit: "ft."});
		return `${speed} ${unit}`;
	}
	// endregion
	...
	static _getShipMovement_infwar (veh) {
		...
		if (veh.speed) {
			out.walk = Config.getMetricNumberDistance({configGroup: "importVehicle", originalUnit: UNT_FEET, originalValue: veh.speed});
			out.units = Config.getMetricUnitDistance({configGroup: "importVehicle", originalUnit: "ft"});
		}
