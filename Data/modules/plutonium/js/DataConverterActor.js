...
class DataConverterActor extends DataConverter {
	...
	static _getParsedActionEntryData_hit ({entity, action, dataBuilderOpts, mode, state}) {
		...
		const hitBonusFromAbil = toHitNumerical - enchBonus - (dataBuilderOpts.assumedPb || 0);
		...
		if (mode === "creature") {
			...
			if (!isFoundOffensiveAbility && dataBuilderOpts.assumedPb) {
				...
				const curCalcBonus = dataBuilderOpts.assumedPb + Parser.getAbilityModNumber(entity[state.offensiveAbility]) + state.attackBonus;
