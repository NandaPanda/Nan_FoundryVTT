...
class DataConverterSpell extends DataConverter {
	...
	static async pGetSpellItem (spell, opts) {
		...
		const state = new this._PassiveEntryParseStateSpell(spell, opts);
		...
		const img = await this._pGetSaveImagePath(spell, {propCompendium: "spell"});
	...
	static _pGetSpellItem_mutConsumes ({spell, opts, state}) {
		...
		if (resource === ConfigConsts.C_SPELL_POINTS_RESOURCE__SHEET_ITEM) {
			if (state.consumeType === undefined) state.consumeAmount = "charges";
			if (state.consumeAmount === undefined) state.consumeType = consumeAmount;
			if (state.consumeTarget === undefined) state.consumeTarget = opts.spellPointsItemId;
			return;
		}

		if (state.consumeType === undefined) state.consumeAmount = "attribute";
		if (state.consumeAmount === undefined) state.consumeType = consumeAmount;
