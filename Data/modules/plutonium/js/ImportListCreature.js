...
class ImportListCreature extends ImportListActor {
	static ID = "creatures";
	static DISPLAY_NAME_TYPE_PLURAL = "Creatures";
	...
	async _pImportEntry_pFillItems_pSpellcasting (mon, act, isTemporary, monOpts) {
		...
		for (const spellcasting of mon.spellcasting) {
			...
			if (cpySpellcasting.charges) {
				const consumeTarget = this._getChargesSheetItem(cpySpellcasting, monOpts);
		...
		for (const spellcastingUnknownProps of spellcastingsUnknownProps) {
			await this.constructor._pGetSpellHashToItemPosMapAndFillSpells(
				...
				JSON.stringify(spellcastingUnknownProps),
				{
					...
					isTemporary,
					optsGetSpellItem: {
						ability: spellcastingUnknownProps.ability || monOpts.spellAbility,
					},
				},
			);
		}
...
ImportListCreature.ImportCustomizer = class extends ImportCustomizer {
	...
	_activateListeners_bindControls ({$html, $wrpBtnsSort}) {
		...
		$html.find(`[name="btn-run"]`).click(async () => {
			...
			const scaledMons = await Promise.all(listValues.map(async ({ix, targetCr, targetSpellLevel, targetClassLevel, ixVariant, rename, isNamedCreature, ixToken, tokenCustom}) => {
				...
				if (rename) mon = await ImportListCreature.ImportCustomizer._pGetRenamedMon(mon, {newName: rename, isNamedCreature});
	...
	static _pGetRenamedMon (mon, {newName, isNamedCreature}) {
		...
		ImportListCreature.ImportCustomizer._PROPS.forEach(prop => applyTo(prop));
	...
	static _getPossibleMonShortName (mon) {
		...
		ImportListCreature.ImportCustomizer._PROPS.forEach(prop => {
			if (!mon[prop]) return;
...
ImportListCreature.ImportCustomizer._PROPS = [
	"action",
	"bonus",
	"reaction",
	"trait",
...
ImportListCreature.ImportCustomizer._RE_SHORTNAME_PT_VERB = `(?:is|can|starts|has|takes|targets|exhales|fails|makes)`;
ImportListCreature.ImportCustomizer._RE_SHORTNAME_PT_NAME = `(?<name>[a-z ]+)`;
ImportListCreature.ImportCustomizer._RE_SHORTNAME__IF_THE = new RegExp(`If the ${ImportListCreature.ImportCustomizer._RE_SHORTNAME_PT_NAME} ${ImportListCreature.ImportCustomizer._RE_SHORTNAME_PT_VERB}`);
ImportListCreature.ImportCustomizer._RE_SHORTNAME__THE = new RegExp(`The ${ImportListCreature.ImportCustomizer._RE_SHORTNAME_PT_NAME} ${ImportListCreature.ImportCustomizer._RE_SHORTNAME_PT_VERB}`);
ImportListCreature.ImportCustomizer._RES_SHORTNAME = [
