...
class ImportList extends MiscUtil.mix(Application).with(MixinFolderPathBuilder) {
	...
	static ID = "";
	static DISPLAY_NAME_TYPE_PLURAL = "";
	...
	activateListeners ($html) {
		...
		if (this._pageFilter) {
			pInitialising = this._activateListeners_pInitFilteredList()
				.then(() => this._activateListeners_initPreviewsAndQuicksImportsAndDroppables());
		} else {
	...
	_activateListeners_initPreviewsAndQuicksImportsAndDroppables () {
		...
		for (let i = 0; i < len; ++i) {
			...
			if (!this._isNotDroppable) {
				this._activateListeners_initDroppable(item);
			}
		}
	}
	...
	_activateListeners_initDroppable (listItem) {
