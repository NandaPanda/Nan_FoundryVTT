...
function collectBonusFlags(actor, category, detail) {
	if (!installedModules.get("betterrolls5e")) {
		...
		const bonusFlags = Object.keys(actor.data.flags["midi-qol"]?.optional ?? [])
			...
			if (!checkFlag)
				return false;
			if (!(typeof checkFlag === "string" || checkFlag[detail] || checkFlag["all"]))
				return false;
...
async function bonusCheck(actor, result, category, detail) {
	if (!installedModules.get("betterrolls5e")) {
		...
		*/
		if (bonusFlags.length > 0) {
			...
			switch (category) {
				//@ts-ignore
				case "check":
					title = i18nFormat("DND5E.AbilityPromptTitle", { ability: CONFIG.DND5E.abilities[detail] });
					break;
				//@ts-ignore
				case "save":
					title = i18nFormat("DND5E.SavePromptTitle", { ability: CONFIG.DND5E.abilities[detail] });
					break;
				//@ts-ignore
				case "skill":
					title = i18nFormat("DND5E.SkillPromptTitle", { skill: CONFIG.DND5E.skills[detail] });
					break;
			}
...
export function _prepareOnUseMacroData(wrapped, ...args) {
	...
	if (macros !== undefined)
		...
		setProperty(this.data, "flags.midi-qol.onUseMacroParts", new OnUseMacros(null));
}
