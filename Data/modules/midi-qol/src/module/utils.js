	...
	*/
	if (dmgTypeString !== "") {
		...
		for (let { type, mult } of traitList) {
			...
			if (!magicalDamage && trait.find(t => t === "nonmagic"))
				totalMult = totalMult * mult;
			else if (magicalDamage && trait.find(t => t === "magic"))
				totalMult = totalMult * mult;
...
export async function applyTokenDamage(damageDetail, totalDamage, theTargets, item, saves, options = { existingDamage: [], superSavers: new Set(), semiSuperSavers: new Set(), workflow: undefined }) {
...
export async function applyTokenDamageMany(damageDetailArr, totalDamageArr, theTargets, item, savesArr, options = { existingDamage: [], superSavers: [], semiSuperSavers: [], workflow: undefined }) {
	let damageList = [];
	let targetNames = [];
	let appliedDamage;
...
export async function processDamageRoll(workflow, defaultDamageType) {
	...
	if (workflow.shouldRollOtherDamage) {
		if ((workflow.otherDamageFormula ?? "") !== "" && configSettings.singleConcentrationRoll) {
			appliedDamage = await applyTokenDamageMany([workflow.damageDetail, workflow.otherDamageDetail ?? [], workflow.bonusDamageDetail ?? []], [workflow.damageTotal, workflow.otherDamageTotal ?? 0, workflow.bonusDamageTotal ?? 0], theTargets, item, [new Set(), workflow.saves, new Set()], {
				existingDamage: [],
			...
			if (workflow.otherDamageRoll) {
				...
				appliedDamage = await applyTokenDamage(workflow.otherDamageDetail, workflow.otherDamageTotal, theTargets, item, workflow.saves, { existingDamage: appliedDamage, superSavers: workflow.superSavers, semiSuperSavers: workflow.semiSuperSavers, workflow });
			}
		}
	}
	else {
...
export function checkIncapcitated(actor, item = undefined, event) {
...
function itemReaction(item, triggerType, maxLevel) {
	...
	//@ts-ignore DND5E
	if (item.data.data.attunement === CONFIG.DND5E.attunementTypes.REQUIRED)
		return false;
...
export async function doReactions(target, triggerTokenUuid, attackRoll, triggerType, options = {}) {
	...
	//@ts-ignore attributes
	if (!target.actor || !target.actor.data.flags || target.actor.data.data.attributes.hp.value <= 0)
		return noResult;
	if (checkRule("incapacitated")) {
		try {
			enableNotifications(false);
			if (checkIncapcitated(target.actor, undefined, undefined))
				return noResult;
		}
		finally {
	...
	if (await hasUsedReaction(target.actor) && needsReactionCheck(target.actor))
		return noResult;
...
export async function promptReactions(tokenUuid, triggerTokenUuid, reactionFlavor, triggerType, options = {}) {
	...
	if (!actor)
		return;
	if (await hasUsedReaction(actor) && needsReactionCheck(actor))
		return false;
		...
		if (expression.includes("@")) {
			...
			for (let [k, v] of Object.entries(workflow)) {
				...
				if (v instanceof Set) {
					...
					copyWorkflow[k] = duplicate(workflow[k].data._source);
				else if (k === "actor")
					copyWorkflow[k] = duplicate(workflow[k].data._source);
				else {
					try {
...
export async function removeReactionUsed(actor) {
...
export async function removeBonusActionUsed(actor) {
